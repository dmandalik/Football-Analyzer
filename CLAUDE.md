...
- Elevation interval coverage sits at ~60% (nominal 68%) after whitening + dof inflation. Two candidate causes, deliberately both kept open: (a) the shared box-center systematic the bootstrap cannot resample, (b) non-Gaussian posterior shape from the depth degeneracy. MCMC on real data may subsume either — do not anchor on one.
- Before any real Messi fit: run the 80-realization clip-matched validation at his MEASURED noise point — now known to be ISOTROPIC ~1.3 px detection + ~2.2 px stabilization drift (effective ~2.6 px; the old 1.8/19 ratio-10.8 figures were pan artifact). Anisotropy is a motion-blur phenomenon: slow-motion replays (fast shutter) are isotropic; only live-speed footage gets the blur ellipse. Messi's replay is ~2.5-3x slow motion — the time-scale ruling (as for Çalhanoğlu) applies; his source video contains the live-speed kick as a true-time alternative.
- The ballistic guard is CIRCULAR on RANSAC-parabola-selected detections (the selector guarantees a parabola) — for those, verification must be visual per-frame crops; the guard is only meaningful for independent trackers (SAM 2, TrackNet).
- Messi clip: measurement precision is dominated by STABILIZATION ERROR (2.2 px median drift) not detection noise (1.3 px) — this goes in any writeup. The tight replay camera resists calibration: PnLCalib fails on every frame, and manual goal-plane calibration is focal-depth degenerate (near-fronto-parallel plane; the one off-plane landmark is a line of uncertain identity). The first end-to-end fit ran and FAILED VISIBLY for exactly this reason (280 px residuals, crossing 9.3 m off) — see reports/overlays/messi_liverpool_2019_fit_overlay.mp4. A verified calibration path for tight replay cameras is THE open blocker for the first trustworthy real fit; the slow-mo factor also hit its bound compensating the bad camera, so its 5.14 estimate is meaningless. The (1.6/4.9) clip-matched validation characterizes the solver at that synthetic noise point only — it is NOT validated for any real clip; no verified clip currently sits at that noise point (the Ronaldo figures it was matched to came from a garbage track).
- REQUIRED track verification, no exceptions: (1) full-rate overlay video, watched — contact sheets produced a false positive on Ronaldo (an arc was seen that the numbers contradict); (2) automated ballistic guard — a real flight must show vertical image motion with roughly constant curvature (gravity); flag flat or non-curving tracks (tracking.ballistic_check). Mask-area guards alone pass wrong tracks silently.
- Broadcast cameras follow the ball: per-frame ECC stabilization to a reference frame is STANDARD pipeline, not a special case. Stabilization error compounds over 60+ frames and is part of the noise model; the anisotropy axes must be re-checked after stabilization rather than assumed track-aligned.
- Messi/Çalhanoğlu noise figures predate the verification fix: Messi's 1.8 px cross-track is provisional pending re-verification; Çalhanoğlu (gate-only) equally unverified.
- Compilation trims often start AFTER the kick: check the source for the contact frame before pinning z0 at the first observation (messi2 was 4.5 frames late; pinning at f0 forced 37 m/s + 3300 rpm garbage). Kick time is fractional and MEASURABLE (rest->first-streak displacement over streak velocity); fit takes --kick-frame/--launch-xy, launch anchored at t=0 via simulate_at. Kick-frame +/-0.5 fr is the dominant systematic on such clips - quote its envelope. Pre-trim streak observations (median-bg diff, corner-homography aligned) pin the launch direction and are worth the extra poses.
- load_cameras/smooth_poses/bundle_adjust principal point MUST come from the stored K (clips are 1080p/720p/480p); the 960/540 hardcode silently placed messi2's goal-plane crossing at z=6.9 m. Symptom of a wrong principal point: corner reproj rms ~hundreds of px on every frame while the wireframe render (which used the right K) looks fine.
- VET SOURCES BEFORE INVESTING: TV re-captures carry FRAME BLENDING (50i->30p conversion) - the ball appears as a multi-frame ~600px smear that defeats every compact-object detector. Test on download: triple-diff a mid-action frame; a streak much longer than (speed x shutter) means blended - reject the source. Slow-mo comps also hide interpolation ghosts. Payet is blocked on this after 3 renditions; clean direct-broadcast footage (messi2-like) is the profile to collect.
- Every ball-track candidate must pass the physics sanity BEFORE tiles/overlay effort: implied vertical launch speed <= ~15 m/s, arrival inside the goal quad. Crowd/flag motion chains pass velocity gates but fail these (payet4: 30+ m/s vertical, landed 10m off goal).
- Learned-detector generalization gaps found by leave-one-clip-out so far: (1) motion blur - fixed with directional-blur augmentation (held-out top5 37->48%); (2) BALL COLOR - wp1's yellow winter ball scores 1/17 vs the white-ball-trained detector; hue/saturation jitter goes into the next training run. The auto-track pipeline (track_ball detect/associate) must always run the ballistic guard: it correctly FAILed the two garbage auto-paths (ronaldo 164px, wp1 wandering) while passing messi_live - the system knows when it fails, keep it that way.
- STATIC-UNDER-PAN TEST is mandatory for every manually-read ball position: warp the read pixel through the corner homography into +/-4 neighboring frames and look - a static object (vest, crowd shirt, board logo) stays at the warped point; the ball moves. Both wp1 false anchors (steward vest at f80-84, crowd shirt at f88) would have been caught instantly; instead they poisoned two fits and a search corridor. One command, zero excuse to skip it.
