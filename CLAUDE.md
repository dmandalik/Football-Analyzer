...
- The ballistic guard is CIRCULAR on RANSAC-parabola-selected detections (the selector guarantees a parabola) — for those, verification must be visual per-frame crops; the guard is only meaningful for independent trackers (SAM 2, TrackNet). The (1.6/4.9) clip-matched validation characterizes the solver at that synthetic noise point only — it is NOT validated for any real clip; no verified clip currently sits at that noise point (the Ronaldo figures it was matched to came from a garbage track).
